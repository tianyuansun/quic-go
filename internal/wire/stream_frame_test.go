...
var _ = Describe("STREAM frame", func() {
	Context("when parsing", func() {
		...
		It("errors on EOFs", func() {
			...
			data = append(data, []byte("foobar")...)
			_, err := parseStreamFrame(bytes.NewReader(data), 0x8^0x4^0x2, protocol.Version1)
			Expect(err).NotTo(HaveOccurred())
			for i := range data {
				_, err := parseStreamFrame(bytes.NewReader(data[0:i]), 0x8^0x4^0x2, protocol.Version1)

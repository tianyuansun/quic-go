...
var _ = Describe("Received Packet Handler", func() {
	...
	It("generates ACKs for different packet number spaces", func() {
		...
		Expect(handler.ReceivedPacket(2, protocol.ECT0, protocol.EncryptionInitial, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(1, protocol.ECT1, protocol.EncryptionHandshake, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(5, protocol.ECNCE, protocol.Encryption1RTT, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(3, protocol.ECT0, protocol.EncryptionInitial, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(2, protocol.ECT1, protocol.EncryptionHandshake, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(4, protocol.ECNCE, protocol.Encryption1RTT, sendTime, true, false)).To(Succeed())
	...
	It("uses the same packet number space for 0-RTT and 1-RTT packets", func() {
		...
		sendTime := time.Now().Add(-time.Second)
		Expect(handler.ReceivedPacket(2, protocol.ECNNon, protocol.Encryption0RTT, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(3, protocol.ECNNon, protocol.Encryption1RTT, sendTime, true, false)).To(Succeed())
	...
	It("rejects 0-RTT packets with higher packet numbers than 1-RTT packets", func() {
		...
		sentPackets.EXPECT().GetLowestPacketNotConfirmedAcked().AnyTimes()
		sendTime := time.Now()
		Expect(handler.ReceivedPacket(10, protocol.ECNNon, protocol.Encryption0RTT, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(11, protocol.ECNNon, protocol.Encryption1RTT, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(12, protocol.ECNNon, protocol.Encryption0RTT, sendTime, true, false)).To(MatchError("received packet number 12 on a 0-RTT packet after receiving 11 on a 1-RTT packet"))
	})
	...
	It("allows reordered 0-RTT packets", func() {
		...
		sendTime := time.Now()
		Expect(handler.ReceivedPacket(10, protocol.ECNNon, protocol.Encryption0RTT, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(12, protocol.ECNNon, protocol.Encryption1RTT, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(11, protocol.ECNNon, protocol.Encryption0RTT, sendTime, true, false)).To(Succeed())
	})

	It("drops Initial packets", func() {
		sentPackets.EXPECT().ReceivedPacket(gomock.Any()).Times(2)
		sendTime := time.Now().Add(-time.Second)
		Expect(handler.ReceivedPacket(2, protocol.ECNNon, protocol.EncryptionInitial, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(1, protocol.ECNNon, protocol.EncryptionHandshake, sendTime, true, false)).To(Succeed())
	...
	It("drops Handshake packets", func() {
		...
		sendTime := time.Now().Add(-time.Second)
		Expect(handler.ReceivedPacket(1, protocol.ECNNon, protocol.EncryptionHandshake, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(2, protocol.ECNNon, protocol.Encryption1RTT, sendTime, true, false)).To(Succeed())
		Expect(handler.GetAckFrame(protocol.EncryptionHandshake, true)).ToNot(BeNil())
		handler.DropPackets(protocol.EncryptionInitial)
	...
	It("drops old ACK ranges", func() {
		...
		Expect(handler.ReceivedPacket(1, protocol.ECNNon, protocol.Encryption1RTT, sendTime, true, false)).To(Succeed())
		Expect(handler.ReceivedPacket(2, protocol.ECNNon, protocol.Encryption1RTT, sendTime, true, false)).To(Succeed())
		ack := handler.GetAckFrame(protocol.Encryption1RTT, true)
		Expect(ack).ToNot(BeNil())
		...
		Expect(handler.ReceivedPacket(4, protocol.ECNNon, protocol.Encryption1RTT, sendTime, true, false)).To(Succeed())
		ack = handler.GetAckFrame(protocol.Encryption1RTT, true)
		Expect(ack).ToNot(BeNil())
	...
	It("says if packets are duplicates", func() {
		...
		Expect(handler.ReceivedPacket(3, protocol.ECNNon, protocol.EncryptionInitial, sendTime, true, false)).To(Succeed())
		...
		Expect(handler.ReceivedPacket(3, protocol.ECNNon, protocol.EncryptionHandshake, sendTime, true, false)).To(Succeed())
		...
		Expect(handler.ReceivedPacket(3, protocol.ECNNon, protocol.Encryption0RTT, sendTime, true, false)).To(Succeed())
